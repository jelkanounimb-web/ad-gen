...
import { 
  ...
  Sparkles, Download, Wand2, Play, Settings, ChevronDown, ChevronUp,
  Palette, Share2, Layers, Eye, Edit3, Globe
} from 'lucide-react';
...
export const LandingPageBuilder: React.FC<LandingPageBuilderProps> = ({ generatedContent }) => {
  ...
  const [activeTab, setActiveTab] = useState<'MAGIC' | 'EDIT'>('MAGIC');
  ...
  useEffect(() => {
    if (generatedContent) {
      ...
      setConfig(prev => ({
        ...
        content: {
          ...prev.content,
          hero: { ...prev.content.hero, headline: generatedContent.hero.headline, subheadline: generatedContent.hero.subheadline, cta: generatedContent.hero.cta, videoKeyword: 'business' },
        ...
        <div className="p-4 lg:p-5 border-b border-white/10">
          ...
          <div className="flex bg-slate-800 p-1 rounded-lg mt-4 border border-white/5">
            <button 
              ...
              className={`flex-1 text-xs font-bold py-2 rounded-md transition-all flex items-center justify-center gap-2 ${activeTab === 'MAGIC' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
            >
              <Wand2 className="h-3.5 w-3.5" /> Magic Input
            </button>
            <button 
              onClick={() => setActiveTab('EDIT')}
              className={`flex-1 text-xs font-bold py-2 rounded-md transition-all flex items-center justify-center gap-2 ${activeTab === 'EDIT' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
            >
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-5 custom-scrollbar pb-24 lg:pb-5">
          ...
          {activeTab === 'MAGIC' ? (
            ...
            <div className="space-y-2 animate-fade-in" dir={isRTL ? 'rtl' : 'ltr'}>
              <AccordionItem title="Hero Section" icon={<Type />} isOpen={openSection === 'hero'} onToggle={() => setOpenSection(openSection === 'hero' ? null : 'hero')}>
                ...
                <InputField label="Video Keyword (Unsplash)" value={config.content.hero.videoKeyword} onChange={(v) => updateContent('hero', 'videoKeyword', v)} />
              </AccordionItem>

              <AccordionItem title="Problem / Pain" icon={<AlertTriangle />} isOpen={openSection === 'problem'} onToggle={() => setOpenSection(openSection === 'problem' ? null : 'problem')}>
                <InputField label="Section Title" value={config.content.problem.title} onChange={(v) => updateContent('problem', 'title', v)} />
              </AccordionItem>

              <AccordionItem title="Features & Benefits" icon={<CheckCircle2 />} isOpen={openSection === 'solution'} onToggle={() => setOpenSection(openSection === 'solution' ? null : 'solution')}>
                <InputField label="Section Title" value={config.content.solution.title} onChange={(v) => updateContent('solution', 'title', v)} />
              </AccordionItem>

              <AccordionItem title="Social Proof" icon={<MessageSquare />} isOpen={openSection === 'testimonials'} onToggle={() => setOpenSection(openSection === 'testimonials' ? null : 'testimonials')}>
                 <InputField label="Section Title" value={config.content.testimonials.title} onChange={(v) => updateContent('testimonials', 'title', v)} />
              </AccordionItem>

              <AccordionItem title="Pricing" icon={<CreditCard />} isOpen={openSection === 'pricing'} onToggle={() => setOpenSection(openSection === 'pricing' ? null : 'pricing')}>
      ...
      `}>
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden p-0 lg:p-8 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] bg-slate-950 flex justify-center pb-24 lg:pb-8">
          ...
          >
            ...
            {/* HERO SECTION */}
            <section className="relative pt-20 pb-20 md:pt-24 md:pb-32 px-6 overflow-hidden">
               ...
               <div className="max-w-4xl mx-auto text-center relative z-10">
                  ...
                  {/* AI VIDEO SIMULATION */}
                  <div className="relative w-full aspect-video rounded-xl md:rounded-2xl overflow-hidden border border-white/10 shadow-2xl group cursor-pointer bg-slate-900">
                     <img 
                       src={`https://source.unsplash.com/1600x900/?${config.content.hero.videoKeyword || 'technology'}`} 
                       alt="AI Video Cover" 
                     ...
                     <div className="absolute inset-0 flex items-center justify-center">
                        <div className="w-16 h-16 md:w-20 md:h-20 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center border border-white/20 group-hover:scale-110 transition-transform shadow-[0_0_30px_rgba(99,102,241,0.4)]">
                           <Play className="h-6 w-6 md:h-8 md:w-8 text-white fill-white ml-1" />
                        </div>
                     </div>

                     <div className="absolute top-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-md border border-white/10 flex items-center gap-2">
                        <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
                        <span className="text-[10px] font-bold text-white uppercase tracking-wide">AI Generated</span>
                     </div>
                     
                     {/* Fake Video UI Controls */}
                     <div className="absolute bottom-0 left-0 right-0 p-4 flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300" dir="ltr">
                        <Play className="h-4 w-4 text-white fill-white" />
                        <div className="h-1 flex-1 bg-white/20 rounded-full overflow-hidden">
                           <div className="h-full w-1/3 bg-indigo-500 rounded-full" />
                        </div>
                        <span className="text-xs font-mono text-white">00:15 / 01:30</span>
                     </div>
                  </div>
               </div>
            </section>
            ...
            {/* SOLUTION / FEATURES */}
            <section className="py-16 md:py-24 px-6 bg-gradient-to-b from-slate-900 to-slate-950 border-t border-white/5">
               <div className="max-w-6xl mx-auto">
